...
- **Deterministic ref selection:** if GitHub reports an ambiguous ref and multiple matching tags are valid for the requested version prefix, the resolver picks the longest valid tag name and uses lexicographic order as a stable tie-breaker. Valid candidates are the exact ref or tags that continue it with `.`, `-`, or `_`; exact branch/tag name conflicts still fail closed.
